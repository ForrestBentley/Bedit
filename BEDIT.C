...
//* Forrest Bentley   02/06/2014 Added Debug semaphore processing        */
//* Forrest Bentley   02/07/2014 Added breakpoints to debug processing   */
//* Forrest Bentley   02/08/2014 Improved Debug Variable data returned   */
//* Forrest Bentley   02/12/2014 Added Debug File Viewing                */
//*                              Added Debug TRUE, FALSE, Contains       */
//* Forrest Bentley   02/15/2014 Added FIXFILE (correct long line files) */
//* Forrest Bentley   02/25/2014 Fixed Abort with files TOO LARGE        */
/*************************************************************************/
...
main (argc, argv)
    ...
    if  (argc > 1) {
        args_supplied = 'y';
        while (--argc) {
            strcpy (szMainTitle, *++argv);
            if  (szMainTitle [0] == '-') {         // Max Characters
                if  (szMainTitle [1] == 'F') {
                    cFast = 'y';
                    //printf ("Fast\n");
                }
                    ...
                    if  (szMainTitle [1] == 'D') {
                        ...
                        if  (szMainTitle[1] == 'E') {
                            //printf ("Encrypted Command File\n");
                            cEncryptCommand = 'y';
                            if  (strlen (szMainTitle) > 3) {
                                strcpy (szEncryptKey, szMainTitle + 3);
                                if  (strcmp (szEncryptKey, "DEFAULT") == 0) {
                                    strcpy (szEncryptKey, szDefaultKey);
                                }
                                //printf ("szEncryptKey["); printf (szEncryptKey); printf ("]\n");
                            }
                        }
                        else {
                            //printf ("MAX Set\n");
                            Number = 0;
                            Number = (long int) strtol ((char *)szMainTitle + 1, (char**)&p, 10);
                            if  ((Number > 99) && (Number < 50000)) {
                                Max = (long int) Number;
                            }
                        }
                    }
